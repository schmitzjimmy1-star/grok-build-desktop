...
| Slice 4C paid matrix | `python3 -m unittest scripts.acceptance.tests.test_4c_paid_lock`; `swift test --filter AcceptanceHarnessTests/testSlice4C`. Native freeze bind is in Swift. Ceiling dispatcher accepts frozen 4C only; schema-3 stays on the 4M refusal. Schema-4 preflight uses the leased pager, not official 1.0.5. Catalog prices are campaign-confirmed. |
| Slice 4B.6 signed candidate install | `python3 -m unittest scripts.acceptance.tests.test_v3_install`; owner-local `swift test --filter Slice4B5LifecycleTests` with `GROKBUILD_SLICE4B3_RUNTIME_SELECTION` aimed at the 4B.5 selection (`…/GrokBuild-Slice4B5/runtime/runtime-selection.json`), never the 4B.3 `14da2ef77…` tree. Local CLI pager tests: `CARGO_TARGET_DIR` must be a no-space path (`/tmp/grokbuild-cargo-target`) and `dotslash` on `PATH`. |
