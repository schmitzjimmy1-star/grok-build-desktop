...
| **4** | **Official provider and open-weight lane** | Use official provider definitions, resolve the keyless-endpoint credential hazard, and pilot one Keychain-backed auth helper without bundling a model runtime. | **4B complete (accepted); 4C locked** |
...
| **4B** | **Activation supply chain and nonbillable lifecycle** | Publish and pin the exact fork candidate, prove non-executable credential materialization and canonical bound provenance, redesign continuation around fresh-process `session/load`, then pass hostile loopback lifecycle acceptance. | **4B.0–4B.6 accepted; CLI PR #7 merged as `7e9f1ad`; paid locked. See activation campaign.** |
