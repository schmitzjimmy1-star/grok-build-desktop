...
| **4B** | **Activation supply chain and nonbillable lifecycle** | Publish and pin the exact fork candidate, prove non-executable credential materialization and canonical bound provenance, redesign continuation around fresh-process `session/load`, then pass hostile loopback lifecycle acceptance. | **4B.0–4B.6 accepted; CLI PR #7 leftover; paid locked. See activation campaign.** |
