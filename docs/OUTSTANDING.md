...
is the GUI / ACP / CLI contract at the repository root. 4C native Sends have
not billed. Campaign notes:
...
exact-head publication PR #113 is the campaign closeout vehicle.
...
This Phase 3 PR was docs-only. It merged into `main` as `90510526faea91219fc79bfe9d9cff3780bf6129` (PR #92).

### Phase 4 receipt — 2026-08-15

PR [#92](https://github.com/schmitzjimmy1-star/grok-build-desktop/pull/92)
merged into `main` as `90510526faea91219fc79bfe9d9cff3780bf6129`. Phase 4
started on branch `codex/grokbuild-c8-p4-cli-104` from that `main`.

Pre-update state recorded at `2026-08-15T02:02:47-0500`:
...
- `~/.grok/config.toml` SHA-256 `2cb4dcdaf0f1841aab54fb2ae10586381ca78b560eb9fc0477efb521e92140ae`,
  2894 bytes, mode `0600`
- Installed GrokBuild Settings → App CLI line: `Installed: 1.0.3`, `Latest: 1.0.4`,
  `Status: Update available.`

Update executed via official `grok update`:
- Output: `Updating Grok 1.0.3 → 1.0.4`, `Downloading grok v1.0.4 (macos-aarch64)...`,
  `✓ grok v1.0.4 installed successfully!`

Post-update verification:
- `grok --version` → `grok 1.0.4 (d846eb93d94d) [stable]`
- `~/.grok/config.toml` SHA-256 `2cb4dcdaf0f1841aab54fb2ae10586381ca78b560eb9fc0477efb521e92140ae`,
  2894 bytes, mode `0600` (bit-for-bit identical; 85 key paths; zero unexpected provider,
  credential, or MCP stanzas).
- `grok inspect --json` reports `grokVersion: "1.0.4"`, `channel: "stable"`, 3 builtin agents,
  13 externalCompat cells.
- GrokBuild Settings → App Check for Updates → `Installed: 1.0.4`, `Latest: 1.0.4`,
  `Status: Up to date.`
- `make test` passed (**865 tests, 0 failures** in 36.331 s) against unchanged installed app
  `27c146e`. Zero test regressions.

Frozen native no-tool marker packet:
- Marker: `GB-C8-P4-CLI104-20260815T070258Z`
...
- Assistant response: `GB_C8_P4_CLI104_RECEIPT_OK`
- Run Inspector & session metadata receipt:
  - Model: `grok-4.6` (`grok-4.6-build`)
  - Reasoning effort: `low` (1s thought, 38 reasoning tokens)
  - Route: `Route: native xAI through the Grok CLI.`
  - Process generation: 1
  - Session UUID: `01a0043b-886a-7e73-96eb-d66a9d4d5d9b`
  - Usage: 15,984 tokens (15,927 in • 57 out • 6,144 cached read • 38 reasoning)
  - Cost: $0.04 ($0.039066)
  - Duration: 35.5s, 1 model call, 0 tools

Gate F cleanup & process-zero:
- `grok sessions delete 01a0043b-886a-7e73-96eb-d66a9d4d5d9b` deleted backend directory;
  `grok sessions search GB-C8-P4-` returned `Total: 0`.
...
### Slice 4 receipt — inspector dock + horizon clear, 2026-08-13

Repair is on `codex/grokbuild-audit-s4-inspector-clear` at `4d21aac`. Candidate
Computer Use ran on `/Applications/GrokBuild.app` stamped `4d21aac` (`dirty=false`),
Team `DD2GCQJVB4`, SHA-256
`66e9e2be0393067e9b8d1658252b5b6074067f37bf13670641046480e5fdc0f4`.
`make test` **786/786**. Computer Use via `agent-desktop` session
`run-1786672043765-43032-0`. Ceiling **150k**; charged **91.8k** actual tokens
(6 calls / 3 turns / $1.45).

**Code**

- `inspectorDockMinimumChatWidth` is **1100**. Default 1440×900 docks. Overlay
  remains only for 900..<1100. Below 900 an open inspector is a 40-pt collapsed
  strip. Header **Run inspector** vs sidebar bell **Session dashboard**.
- `BackgroundTaskTracker.beginUserTurn()` (from `clearTurnState`) prunes
  non-scheduled live workers; scheduled tasks stay. Compact tool-row AX labels
  include bounded settled output. README no longer claims first typing warms
  the agent.

**Default window.** Main window 1440×819 (menu-bar clipped height). Open Run
inspector: panel `grok-activity-sidebar` at x=1180 w=260; composer right edge
1089.5; **no overlap**. Bell AX **Session dashboard**. Toggle **Hide run inspector**.

**GROK-HORIZON.** Native Grok 4.6, Direct xAI, no fallback. T1 spawned two
lanes (`grok-inspector-subagents` **2 running** then **2 done**). T2 and T3
no-tool markers `GB-S4-HORIZON-T2-20260813T2048` /
`GB-S4-HORIZON-T3-20260813T2048`. After T3, inspector had **no**
...
control wired to a real owner, tool use obvious, the GitHub-looking panel that
appears after Send gone, and Grok / OpenRouter / OpenAI packets that do not
leak credentials or stir idle CPU. Billable prompts and retries are authorized
for acceptance. Same gates A–H, one PR per owner. Do not invent provider
fallback.
...
| 3 | Readiness copy: fresh start must not say Resuming saved task; unsent Ready must not contradict idle sidebar | Low | None unless a live turn later lies | Merged PR #55 `fab098d` |
...
on `main`. GrokBuild is a thin ACP client; the CLI owns the agent; GUI steps
name ACP methods. Paid 4C Sends have **not** billed. Do not unlock
`_billable_v3`, do not `grok update`, and do not rebuild pager
`1.0.5 (8226242)`. Visual Quiet leftover Phase 3 (`ChatView` split) stays
unauthorized. Re-derive canonical identity before any future work.
