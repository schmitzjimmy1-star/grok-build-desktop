...
| Same-session plain model switches | **Pass** — a fresh disposable session completed exact markers while switching GPT-4.1 Mini → Terra → Grok; backend history recorded the expected provider-facing model for each plain turn. |
...
## Visual UX Slice 9 — quiet workbench focus acceptance (2026-08-02)

| Check | Installed-app result |
|---|---|
| Fresh-work hierarchy | **Pass** — the signed app led with **What would you like to do?**, an explicit pre-send model menu, and exactly Ask, Build, and Review. Each card explained an outcome and filled a normal editable draft without sending or starting a backend. |
| Keyboard / accessibility | **Pass** — Shift-Tab reached Review, Space filled its draft, and the same path reached and opened `grok-starter-model-selector`. Intent cards, composer, model menu, and the Details disclosure exposed stable labels, values, hints, and identifiers. |
| Progressive developer power | **Pass** — the primary composer row stayed quiet. One Details click exposed Agent mode, the full model/reasoning selector, context, changed files, Activity, branch/worktree, Browser Tools, and Computer Use; none of those capabilities was removed. |
| Model selection truth | **Pass** — send-free installed acceptance selected Terra and observed a saved/no-process receipt, then restored Grok 4.5. Jimmy's later weather turn independently reported live confirmed `gpt-5.6-terra`; backend `turn_started`, `turn_completed`, and model-usage records agreed on Terra with 23,677 tokens, two model calls, one web search, and no unresolved tool failure. |
| Automated verification | **Pass** — `WorkbenchIntentTests` completed **5/5** and `make test` completed **557/557** with zero failures in 21.713 seconds; `git diff --check` is part of final packaging. |
| Durable user state | **Pass** — the user-authored Terra turn legitimately advanced the baseline to 207 transcript files and 257 Grok histories. Every file remained byte-identical through final packaging, quit, exact installed relaunch, transcript restore, and a send-free new-session pass; GrokBuild never rewrote the durable logs. |
| Signing / exact install / relaunch | **Pass** — dist/install executable SHA-256 both equal `e7b1222605d16f1071baf35cb4d98bfb04d1eb9a31d5e481fd2d8b635760d5db`; both bundles pass deep/strict verification under `Apple Development: jhschmitz1993@gmail.com (LS4SUB57QL)` / Team `DD2GCQJVB4`. The installed stamp is personal repository `schmitzjimmy1-star/grok-build-desktop`, `main`, `e1f0ed4b7444511ddc16878ef7829eb34fb37928`, dirty `true`. Exact relaunch restored the weather transcript, then New Session exposed the Slice 9 welcome. Five settled samples were 0.0% CPU at 122,528 KB RSS with zero children. Rollback: `/Users/jimmyschmitz/.Trash/GrokBuild-pre-slice9-20260802-2208.app`. |

## Visual UX Slice 10 — glanceable live progress and repeated-turn bottom-follow acceptance (2026-08-02)

| Check | Installed-app result |
|---|---|
| Compact live truth | **Pass** — turns two and three exposed accessible `grok-live-progress` within 1,236 ms and 891 ms of Return. The line named Thinking, 0 active workers, and event-driven elapsed time, derived from the same generation-bound projection as Activity. Source and focused tests exclude budget, token, usage, and outcome copy. |
| Start latency | **Pass with cold-start receipt** — Return-to-backend-turn-start was 3,545 ms for the first fresh-process turn and 80 ms / 71 ms for warm turns two and three. Backend stream-start followed Return by 4,095 ms / 507 ms / 495 ms. All three authoritative receipts named one backend and Grok 4.5. |
| Streaming truth | **Pass** — the provider still supplied each short answer as one final ACP message chunk, but GrokBuild now reveals a large chunk across a bounded fourteen display frames and defers successful-completion reconciliation until that buffer drains. A fake-ACP integration proves partial first paint, continued live state, exact final content, and preserved thinking under an immediate completion receipt; this remains honestly described as client-side presentation pacing rather than provider token streaming. |
| Thinking and live-status order | **Pass** — every active assistant turn uses one explicit order: thinking, tool activity, live progress, then answer. The regression was expanded after native screenshot evidence showed `Writing answer` beneath a partial answer. In the rebuilt installed app, the changing accessibility tree was `Show thinking` → `Live progress` → partial `Build agent`; the captured native frame visibly placed `Writing answer · 0 active workers · 10s elapsed` above the growing nine-item answer. |
| Turns two and three | **Pass** — exact markers `SLICE10_STREAM_2_OK` and `SLICE10_STREAM_3_OK` were fully visible immediately above the empty composer with no manual scroll. The 3.1-second bounded layout-settlement schedule covers delayed rich-height changes without overriding a reader who scrolls away. |
| Same-thread restoration | **Pass** — one backend `019fc5b5-06a1-7c73-b351-d8231fcc33b8` completed all three prompts with `end_turn`; exact Command-Q/relaunch restored the three local turns and the third marker at the bottom, then correctly blocked a new send pending continuity verification. |
| Automated verification | **Pass** — focused transcript tests 16/16, focused Activity tests 11/11, live single-chunk ACP integration 1/1, and complete `make test` 562/562 in 22.180 seconds, all with zero failures. |
| Native completion / relaunch | **Pass** — the rebuilt installed app progressively revealed the final proof answer, settled exact marker `LIVE PROGRESS ABOVE ANSWER OK` fully above the empty composer, then Command-Q/relaunch restored that marker and correctly blocked another send pending continuity verification. |
| Durable state | **Pass** — all 207 pre-existing transcript files and 257 pre-existing Grok `chat_history.jsonl` files matched their pre-slice SHA-256 manifests after the final provider proof and exact relaunch. Across Slice 10 and its authorized follow-ups, three new verification threads brought current counts to 213 transcript/metadata files and 260 Grok histories; no pre-existing durable log was rewritten. |
| Signing / exact install | **Pass** — final dist/install executable SHA-256 `5bd7a51579cade63f9c686895c51693b97ad5e20bec9e12b8b5226a568be0726`; both bundles passed deep/strict signing under `Apple Development: jhschmitz1993@gmail.com (LS4SUB57QL)` / Team `DD2GCQJVB4`; quarantine is absent. Installed source is personal `main` at `e7ad9525fc11ad7261eb342235655f15b7e1c644`, dirty `true`. Immediate rollback: `/Users/jimmyschmitz/.Trash/GrokBuild-pre-live-progress-order-20260802-230353.app`. |

## Visual UX Slice 11 — prompt MCPs and compact agent disclosure acceptance (2026-08-03)

| Check | Installed-app result |
|---|---|
| Composer hierarchy | **Pass** — MCP connections sits immediately left of the hammer, followed by Details and the model indicator. A fresh tab restored one secret-free `chrome-devtools` inventory row and attaching it created one removable per-prompt chip. |
| Truthful MCP use | **Pass** — attachment alone made no use claim. The final native turn called `chrome-devtools__list_pages` exactly once, completed marker `SLICE11_FINAL_ALIGNED_MCP_OK`, and the authoritative qualified receipt rendered **Using chrome-devtools**. |
| Compact thinking / answer order | **Pass** — expanding **Build agent** rendered `Thought for 5s` without the verbose running reasoning body, then tool/MCP rows, then the answer. The answer is no longer pushed out of the viewport by streaming thought text. |
| Relaunch persistence | **Pass** — exact Command-Q/relaunch restored the completed turn collapsed; reopening **Build agent** restored the same compact duration, named MCP receipt, qualified tool, and settled answer in the same vertical order. |
...
| Check | Installed-app result |
|---|---|
| Quiet launch choices | **Pass** — the installed candidate distinguished **Resume current task**, **Start new task**, and **Browse old tasks** with named keyboard/VoiceOver controls. The retired foreground banner `Resuming saved session. Send to continue.` was absent. Three external cold launch-plus-AX samples were 10.062/9.720/10.683 seconds; three warm Command-N samples were 0.966/2.786/0.844 seconds. These outer automation measurements are not substituted for the internal first-window signpost budget. |
| Composer and MCP truth | **Pass** — ordinary and provider turns visibly reported `0 MCPs attached`. Browser and Computer Use were enabled only for their exact per-thread probes, named in the task contract and authoritative capability/MCP receipts, then removed with the sessions. No persistent MCP survived relaunch or cleanup. |
| Accessibility and responsive surfaces | **Pass** — keyboard Command-N, the named Browse shortcut, AX names/hints, wide/dark, narrow/dark, and wide/light rendering were exercised in the installed app; the saved theme was restored to Dark. Reduced-motion behavior is pinned by the source/automated contract rather than a global macOS preference mutation. |
| Native sustained task | **Pass** — Grok 4.5 Medium, direct xAI through Grok CLI, no fallback. Parent `019fe99b-44a6-7013-945a-abf04ea07da2` completed four typed steps, two parallel workers (`019fe99b-db9c-7243-ba75-dfbdeafd9825`, `019fe99b-db9d-7da2-a0ca-e0dc60fd3e12`), exact `result.txt`/`artifact.md`, one combined check/status terminal receipt, and `GB-S12-CHECK-PASS`. Review showed the exact two files, `main @ 03d23a3`, All changes scope, safe-revert affordances, and explicit no-auto-publish copy. |
| Stop, resume, and recovery | **Pass** — Stop interrupted one exact 20-second terminal call and settled `Stopped by you` without false completion. Resume used the exact CLI-owned saved-session action; when continuity refused the stopped backend, **Continue as New** created backend `019fe99d-ad68-7a81-9304-d965ccf28d6c` and settled `GB-S12-CONTINUED-SAFELY`. |
| OpenRouter | **Pass** — pinned `openai/gpt-4.1-mini`, Medium, brokered by `openrouter.ai`, no fallback, downstream serving provider unproven. One `./check.sh` receipt settled `GB-S12-OPENROUTER-DONE`; usage `20.9k` tokens / `2` calls / `1` turn, estimated cost shown as approximately `$0.0084-$0.03`. |
| Browser spot check | **Pass** — one exact per-thread Browser capability performed discovery, opened the local fixture page, captured a browser snapshot, and settled `GB-S12-BROWSER-DONE: GB-S12-BROWSER-MARKER`; usage `67.3k` tokens / `4` calls / `1` turn. |
| Computer Use spot check | **Pass** — one exact per-thread Computer Use capability performed discovery and one semantic snapshot, then settled `GB-S12-COMPUTER-DONE GrokBuild`; usage `52.6k` tokens / `3` calls / `1` turn. |
| Context and release-budget truth | **Pass** — measured system instructions, project context, and user content are separate categories; opaque Grok/system/provider wrappers are explicitly **unmeasured**, never guessed. Evidence-backed test gates cover cold first-window, first intent, cold/warm dispatch-to-first-chunk, idle helper/process count, and minimal terminal tokens; they are release assertions, not a second runtime scheduler. |
| Automated verification | **Pass** — focused product-closeout/lifecycle coverage plus complete `make test` **738/738**, zero failures; candidate `make ship` passed. |
| Spend and exact cleanup | **Pass with upstream gap recorded** — accepted usage was approximately `294.8k` tokens / `21` reported calls, below the `1,000,000` cap (the stopped turn exposed no token receipt). All four local sessions were closed in the installed UI. Grok CLI deleted the native parent; recovery and child IDs returned `No session found`, so no private storage was scraped. The exact six-file fixture is recoverable at `~/.Trash/GrokBuild-Slice12-fixture-20260809T233000`. |
| Signing and process zero | **Pass** — candidate installed/dist executable SHA-256 `f315b63d0aed7896feb18100b74cee00f9275c9138a2fe72cd1bc76e43859f37`; deep/strict Team `DD2GCQJVB4`; normal in-app quit left zero exact GrokBuild, grok, Computer Use helper, agent-desktop, or owned-browser-profile processes. |

## Slices 1-5 model-routing synchronization repair acceptance (2026-08-10)

| Check | Installed-app result |
|---|---|
| Saved and live model truth | **Pass** — unavailable explicit saved models fail closed; live switching requires exact allowed ACP effective-model readback and is unavailable while streaming or after assistant history. The installed route disclosure showed **Live model Grok 4.5**, native xAI through Grok CLI, no alternate fallback, the exact backend/process generation/PID, and no stale launch-provider pin. |
| MCP default-off | **Pass** — backend `019fea9d-f276-7553-a562-bb04fd40c0f6` attempted `chrome-devtools__list_pages` without attachment. The app rejected Grok's qualified ACP permission request before Always Approve/YOLO policy could override the gate; the tool visibly failed and the turn settled **Turn cancelled** / **Cancelled checkpoint**, not completion or a transport error. Raw receipt: `stop_reason:cancelled`, `grok-4.5-build`, 15,403 tokens, 1 model call, `costUsdTicks=312964000`. |
| Explicit per-thread MCP | **Pass** — fresh backend `019feaa1-5059-7521-99e0-41f689ab046c` attached `chrome-devtools` before launch; discovery and List Pages succeeded, marker `GB_MCP_FINAL_EXPLICIT_OK_0810` settled `stop_reason:end_turn`, and the attachment cleared after settlement. The receipt showed no app-injected MCP servers and observed CLI-configured `chrome-devtools`. Raw usage: 48,945 tokens, 3 model calls, `numTurns=3`, `costUsdTicks=345796000`; the UI correctly retained one local settled turn. |
| ACP terminal settlement | **Pass** — error completion is a redacted failure; cancelled completion remains distinct from user Stop and releases the one owned prompt continuation when Grok CLI omits the matching `session/prompt` RPC response. The cancelled task returned to a ready composer without a bogus `Failed to send to grok` banner. |
| Cancelled-backend recovery | **Pass with retained interaction issue** — toggling MCP on the cancelled task failed safe because Grok CLI could not resume the backend and exposed **Fresh thread required** / **Continue as New**. Continue-as-New records recovery and requires the subsequent send; this is retained for the read-only Slice 10 review, not described as a same-session success. |
| Automated verification | **Pass** — complete candidate and merged-main runs passed **751/751**, zero failures. Ready implementation PR [#45](https://github.com/schmitzjimmy1-star/grok-build-desktop/pull/45) was reviewed at exact head `2486daef0cd717e6d5891df7b2a47e4e83d717f9` and merged normally as `03ab4f6efa20c31af67d967a29e58e221bc4474d`. |
| Signing and process zero | **Pass** — merged-main install stamp `03ab4f6efa20c31af67d967a29e58e221bc4474d`, `dirty=false`, installed/dist executable SHA-256 `1bf2de27ad8e71b1c4320e2273c1484b47db9c7cfb652c6a4f529df9303b6f72`, Team `DD2GCQJVB4`, deep/strict valid, no quarantine, and zero scoped owned processes after normal quit. |
| Cleanup boundary | **Disclosed** — no acceptance session was deleted. The exact parent IDs and markers remain because the documented Grok CLI child/session visibility and deletion gap is still unresolved; no private-store scrape, daemon, wrapper, broad deletion, or fabricated cleanup proof was used. |

## Slices 6-10 model-routing synchronization repair acceptance (2026-08-10)

| Check | Installed-app result |
|---|---|
| Worker model truth | **Pass** — the runtime model from typed ACP worker lifecycle events is retained and presented independently from the configured role route: `Ran on <model> (Grok ACP)` versus `Routes to <model> (configured)`. A configured route is no longer treated as proof of which model executed. |
| Exact MCP authorization | **Pass** — each MCP-enabled launch refreshes the Grok CLI catalog before provider dispatch, denies every configured-but-unselected server at the CLI boundary, and independently denies generation-mismatched ACP invocations. No-selection retains the catch-all deny. Grok CLI/ACP remains the sole executor and MCP authority; the app only freezes and enforces the user's exact per-thread selection. |
| Authoritative usage and duration | **Pass** — input/output/cached/reasoning/total tokens, per-model splits, API duration, and provider cost survive ACP parsing and settlement. Provider-reported cost wins over a catalog estimate. Tool duration appears only when ACP supplies one of the four accepted millisecond keys; unknown duration stays unknown. |
| Pending-submit freeze | **Pass** — the app latches the exact draft, model, mode, MCP set, and attachments before process startup/resume; route-changing controls stay locked until the intent dispatches or fails, and the exact route is revalidated before provider dispatch. |
| Durable per-turn spine | **Pass** — settled checkpoints persist plan steps, full worker receipts, artifacts, Git-review paths, warnings, and authoritative tool duration. Relaunch can render each assistant turn's own settled spine, and the task header uses a saved checkpoint model only while no process is active. |
| Installed routing controls | **Pass** — the clean installed candidate launched normally. The saved task exposed `grok-4.5 · saved checkpoint`; the model menu showed direct xAI through Grok CLI and no alternate fallback; Add Context showed `0 MCPs attached`, the configured server unselected, and Browser/Computer Use off for this thread. No prompt was sent. |
| Automated verification | **Pass** — implementation commit `15d4bdd7715fa9d11e216d160876f06abed5425e`; complete `make test` and `make ship` passed **759/759**, zero failures. |
| Signing and process zero | **Pass** — installed stamp matched the clean implementation commit with `dirty=false`; dist/installed executable SHA-256 `3b3441a56807e8556b8d9c5821586ff315b86075191ff55d8d3e5fef2ab82baf`; Team `DD2GCQJVB4`; deep/strict valid; no quarantine; normal quit left zero GrokBuild, Grok CLI, Computer Use helper, or agent-desktop processes. |
| Spend and cleanup boundary | **Pass / disclosed** — zero provider prompts and zero provider spend. No acceptance session was deleted. The documented Grok CLI child/session visibility and deletion gap remains open; no private-store scrape, wrapper, daemon, broad deletion, or invented proof was used. |

## Full model-routing refresher acceptance (2026-08-10)

| Installed-app probe | Result |
|---|---|
| Default-off MCP | **Pass** — `GB_REFRESH_DEFAULT_OFF_0810` ran with `0 MCPs attached`; `chrome-devtools__list_pages` was blocked and the turn cancelled instead of leaking a configured capability into the effective route. Local `89FD8A48-2EFC-40CD-AE4B-EE88AFEFDF87`; backend `019fec48-d59e-7d22-96c4-1c5e237b046b`. |
| Exact explicit MCP | **Pass** — `GB_REFRESH_EXPLICIT_MCP_0810` attached only `chrome-devtools`, discovered and exercised `chrome-devtools__list_pages`, returned `GB_REFRESH_EXPLICIT_MCP_OK`, and cleared the attachment after settlement. Local `13101255-177A-45C3-B62D-01E9EF94CE12`; backend `019fec49-def9-70e2-92c5-0985fe878675`. |
| Native plan and parallel workers | **Pass** — `GB_REFRESH_WORKERS_0810` settled its typed plan 3/3 and two concurrent Grok ACP workers. Each exposed the real runtime model, child identity, ACP duration, one tool, and `1/1 succeeded`; the parent tool retained the truthful no-duration state. Local `8C47955D-7FA8-4366-A582-CC2EB20D5D40`; parent `019fec4a-a6a3-75b1-bd1a-04162038b62a`; children `019fec4a-dbdc-7d91-9a09-fc807a472c1c` and `019fec4a-dbde-7d13-b73f-c6873df61c09`. |
| Custom Terra route | **Pass** — the fresh selector, live route, terminal result, final `GB_REFRESH_TERRA_OK`, and settled model split all agreed on `gpt-5.6-terra`; no fallback appeared. Local `CB06F6DC-17B5-4DE4-BEBD-A03D19D2C966`; backend `019fec4b-c7bb-7902-b95f-6b95e5cd6632`. |
| Child-receipt relaunch regression | **Found and fixed** — the refresher caught child outcomes disappearing after relaunch. Commit `734f8980ba212a4664ba5bc27205c5ffc03aeb93` persists and restores `childToolReceipts`. `GB_REFRESH_PERSISTENCE_FIX_0810` retained `Child receipts: 1/1 succeeded`, `grok-4.5 (Grok ACP)`, `3.4 sec`, one tool, and the same identities both before quit and after process-zero/relaunch/restore. Local `A9DCCCD3-9FE7-42C4-99BD-9A37392198B3`; parent `019fec52-7910-7192-be49-b27eb13eb398`; child `019fec52-8ded-7f11-a633-94c23179d581`. |
| Usage | **Pass / exact** — 273,391 total tokens and 20 model calls: 270,923 input, 2,468 output, 135,557 cached-read, and 1,307 reasoning. Grok-backed probes reported $2.995644; Terra returned no provider-cost receipt, so the UI and ledger do not fabricate one. Total usage was 5.47% of the authorized ceiling. |
| Automated and installed gates | **Pass** — focused routing/spine/transcript suites and complete `make test` passed **759/759**. Candidate `make ship` passed with a clean embedded stamp, Team `DD2GCQJVB4`, deep/strict validity, no quarantine, and installed/dist executable parity. |
| Authority and cleanup | **Pass / gap retained** — every probe kept Grok CLI/ACP as executor, froze the exact route, and left MCPs default-off per thread. The five exact local tabs were closed through native session actions; parent, child, and local IDs remain recorded. Exact CLI deletion attempts for all five parent and three child IDs each returned `No session found with id ...`, directly preserving the child/session visibility and deletion limitation instead of papering it over. No private-store scraper, broad deletion, wrapper, daemon, or synthetic cleanup proof was added. |

## Race and checkpoint stress acceptance (2026-08-10)

| Stress boundary | Installed-app result |
|---|---|
| Pending-submit freeze | **Found and fixed** — the exact draft, model, mode, reasoning effort, files, and MCP set now stay latched through startup/restart awaits and are revalidated before dispatch. Missing/wrong model readback and mode mismatch fail before provider spend with the draft intact. The installed saved-session restart failure preserved its 62-character draft, file, and MCP chip and exposed `Fresh thread required`. |
| Model and completion races | **Pass** — unavailable/no-readback/wrong-readback/delayed-readback cases are regression-pinned. Immediate duplicate and prior-turn delayed `turn_completed` replays settle only once and cannot double usage or terminate the next turn. Native/custom routes retain their no-fallback contract. |
| Draft MCP truth | **Found and fixed** — Task Contract now names the exact pending or visible-draft attachment (`chrome-devtools`) when the composer says `1 MCP attached`; it still separates requested, configured, process-ready, discovered, exercised, and successful states. |
| Legacy checkpoint honesty | **Pass** — the pre-repair `GB_PENDING_FREEZE_DISCOVERY_ONLY_OK_0810` checkpoint reopened with a populated Activity surface. Because its older schema never retained usage/process detail, Activity explicitly said `Not reported` and `prior state not retained`; it did not manufacture zeros or success receipts. |
| Full checkpoint relaunch | **Pass** — `GB_CHECKPOINT_RESTORE_OK_0810` survived normal quit, exact process-zero, relaunch, and local transcript restore with identical outcome, model, MCP intent, continuity, tool totals, provider usage/cost/time, and token splits. Local `60E281AE-56E2-44F1-AC70-4BE2B9392F2E`; backend `019feca0-6b27-72b1-b215-ea68476ba4fb`; request `6860eb0a-29cd-4656-b303-964f5ce38f55`. |
| Provider receipt | **Pass / exact** — 15,305 tokens, one call, 15,250 input, 55 output, 5,504 cached-read, 41 reasoning, 1.675 s API time, `grok-4.5-build`, `costUsdTicks=214732000`; zero MCP tool calls. The prior discovery-only call retained 15,329 tokens and $0.22 at installed-UI precision. Combined exact token spend: 30,634 (0.61% of the 5M ceiling). |
| Automated repair gates | **Pass** — implementation commits `ca9a713…`, `79f168b…`, and `8dd0520…`; focused red/green tests plus full `make test`/candidate `make ship` passed **765/765**; `git diff --check` clean. |
| Candidate signing | **Pass** — installed candidate `8dd052068b0f707cfcb5ab3b5ecbbcb289036da6`, `dirty=false`, Team `DD2GCQJVB4`, deep/strict valid, no quarantine, installed/dist SHA-256 `17328f25e5d156c8f8bbfe629cba031924c21d5ee916a1d9360a38848106491c`. |
| Exact cleanup | **Pass / gap retained** — local tabs `D6D645A6-ED27-4D50-B0F2-0C115B46C751` and `60E281AE-56E2-44F1-AC70-4BE2B9392F2E` were closed natively and their local transcript files are absent. Exact `grok sessions delete` attempts for both backend IDs returned `No session found with id ...` (exit 0), which is not deletion proof. No private session storage or non-test history was touched; final candidate process count was zero. |
