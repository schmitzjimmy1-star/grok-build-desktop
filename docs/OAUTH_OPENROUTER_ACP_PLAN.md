...
One interoperability boundary remains explicit. Plain same-session switches among GPT-4.1 Mini, Terra, and Grok passed in a clean disposable session. A separate session that already contained Terra web-search tool calls rejected the first Grok prompt after an in-place switch with `Invalid params`, before any Grok usage or final response was recorded. Provider-specific tool-call history is therefore not portable across every provider boundary. Until GrokBuild can translate or isolate those records safely, users must start a new session before changing provider/model after a web or tool turn. This does not change the successful OAuth, Keychain, catalog, or exact route-readback acceptance above.
