...
- **Custom models (standalone)** — OpenAI-compatible providers and models in **Settings → Models** with device-only Keychain-backed provider secrets, typed catalog validation, redacted diagnostics, native Chat Completions / Responses / Anthropic Messages routing, and atomic owner-only writes to `~/.grok/config.toml`. GrokBuild manages only flat `[model.<id>]` entries. When the file uses advanced Grok structures such as nested model tables or `model_providers`, the pane becomes visibly read-only and leaves the file byte-for-byte unchanged; those entries remain CLI-owned. GrokBuild-only UI hints and credential provenance live in a non-secret sidecar, so the CLI config contains only fields Grok understands. OpenAI preset models default to the Responses API. Usable on its own (no project/session); models are then available in the grok CLI/TUI and in GrokBuild sessions.
