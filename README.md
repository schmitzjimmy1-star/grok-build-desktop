...
- Settings mounts only the selected pane, so hidden diagnostics and loaders stop instead of quietly chewing resources. The Memory pane is the first shared apply-contract reference: its toggle is a real local draft, and explicit status/receipt UI distinguishes Draft, Saved, Restart required, Live, and Unknown. Narrow windows and accessibility text stack its form row instead of crushing the label.
