...
- The native frontend follows the Codex desktop conversation shell: a compact command rail, nested projects and sessions, quiet task header, transcript-first canvas, evidence-gated **Run inspector**, and wide **Describe a task** composer. A new chat offers quiet **Ask**, **Build**, and **Review** starters with a visible outcome; selecting one seeds an editable draft in the selected project and never sends. The starters hide as soon as the composer has a draft. Workspace, branch, and saved-task actions share one context bar above the composer. A fresh empty tab stays idle until Send and shows no dead Run control: typing a draft does not spawn `grok agent stdio`. After Send, the composer and task header say **Starting agent…** / **Connecting** until the live model is confirmed. An idle inherited New chat labels the catalog model **Default**, not Unknown. Unknown is reserved for a receipt conflict or an empty picker. Complete model and receipt controls remain one click away once evidence exists.
