...
- Fresh-turn context accounting is provider-free and honest: GrokBuild measures only the project instructions, skill/MCP catalogs, requested or deferred schemas, history, and memory it can observe, while leaving the Grok CLI's opaque base/provider wrappers unguessed. The maintained project instruction packet is 37% smaller without removing identity, safety, test, signing, or acceptance contracts; MCP tools still use the CLI's progressive discovery path.
