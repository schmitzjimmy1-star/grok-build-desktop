...
- Settings mounts only the selected pane, so hidden diagnostics and loaders stop instead of quietly chewing resources. Agents, Models, Permissions, Memory, Browser, and Computer Use use parent-owned drafts: an edit is never persisted until **Apply**, and each pane distinguishes Draft, Saved, Restart required, Live, and Unknown with a redacted receipt. Future-only defaults say so; launch-affecting changes restart only the current live tab. Narrow windows and accessibility text stack form rows instead of crushing labels.
