...
5. Start a session, choose **Ask**, **Build**, or **Review**, and edit the suggested request—or type your own. Pick your model from the composer's grouped menu (Grok and your providers side by side); reasoning effort, context, usage, and the route/process receipt live in that same model menu.
...
- **Navigation-only sidebar** (Codex parity Slice 1) — the left sidebar is projects and chats: New chat, Sessions, Plugins, and Security actions above pinned projects with nested sessions, search, and the account/settings footer. The header bell opens the activity surface. The former sidebar Activity lane, Agents hub, and Connections lane were removed as permanent sections; their capabilities remain in the Activity drawer and dashboard, the Settings → Agents pane (defaults and custom roles), and the composer's MCP attachment menu respectively — nothing was deleted from the runtime.
- **A compact contextual inspector** (Codex parity Slice 5) — the Activity panel is now a small top-right overlay: subagent counts, Computer Use state with a Settings link, and Sources that separate requested MCPs from ones actually evidenced by tool receipts. The full evidence stack (workers, tools, artifacts, usage, continuity) lives one click away under Run details, with the same strict truth rules; changed files live in the inline card and the Review pane.
