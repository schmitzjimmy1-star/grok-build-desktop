...
**Durable Run history and export:** `RunHistory` is a pure projection of `AssistantTurnCheckpoint` values already attached to persisted assistant messages. The Session dashboard groups checkpoints by their retained parent backend ID, preserves every retained turn boundary, and labels them historical; it does not scrape Grok session directories or mint current-process truth after relaunch. Its Markdown and JSON exporters are deterministic and bounded, and serialize only selected typed checkpoint receipt fields. They omit user/assistant transcript prose, prompt text, tool payloads, artifact paths, environment values, credentials, and private reasoning. New checkpoints preserve the credential-free route detail from the exact process-generation contract; old checkpoints, retries, and parallel tool-group topology remain `not retained`. UI/export code must never infer them from prose or ordering.
...
**Eviction and runtime leases:** `SessionRuntimeRetentionPolicy` is the pure owner of the normal four-process window consumed by `enforceConnectionCap()`. It receives each tab's exact connection state, active-process fact, activation ordinal, selected identity, and generation-bound schedule lease; the four most-recent ordinary live sessions are retained, while starting, busy, and actively scheduled sessions are protected outside those ordinary slots. A selected scheduled session therefore cannot consume an ordinary slot: four ordinary sessions plus that lease produce five live processes, and the Session dashboard shows the exact soft-cap excess instead of silently cancelling work. `ScheduledTaskInventoryReceipt` binds the last authoritative scheduler output to one local tab, backend, process generation, observation time, and task count; only a nonempty exact current receipt mints `SessionRuntimeLease`. Reconnect, Stop, close, quit, or process failure clears the lease, and restored/cached task metadata cannot recreate one until that live generation is observed again. The eviction loop still snapshots candidate IDs, re-resolves after every asynchronous teardown, and accepts a backend receipt only through `SessionProcessLRUPolicy`; a mismatched process is stopped but its identity is never adopted by another tab.
...
returns for a live, stalled, or recovery session. At settlement, `AssistantTurnCheckpoint` copies only
...
**Slice 5 subagent truth (audit campaign).** User **Stop** calls `markActiveSubagentsStoppedByUser()` before `markActiveActivitiesStopped()`: bound subagents without `subagent_finished` become **orphaned**; spawn rows with no `childID` become **cancelled**; non-subagent background commands still use `"stopped"`. `BackgroundTaskTracker.unboundSpawnedEvents` exposes pending `subagent_spawned` receipts that never matched a spawn row; `ChatStore.currentTurnEvidenceWorkers()` folds them into live/settled projections as synthetic workers (`unbound|<childID>`, status `unknown`) without inventing a `BackgroundActivity`. `beginUserTurn()` drops those pending receipts with the prior-turn live mirror so they cannot leak into the next send. `ChildLedgerReadOutcome` (`unreadable` | `empty` | `receipts`) rides on `BackgroundActivity` and `RunEvidenceSnapshot.Worker`; `reconcileChildToolReceipts` and `subagent_finished` set it from `loadChildToolReceipts` nil vs `[]` vs receipts. Presentation (`workerReceiptDetail`, `ContextInspectorProjection.subagentSummary`, `Worker.isUnresolved`) treats orphaned/cancelled/stopped and unreadable ledgers as non-success; `doneCount` checks `isUnresolved` before `isCompleted`.
...
| Store | `ChatStore.backgroundActivities` is the correlated presentation projection; `clearTurnState` calls `BackgroundTaskTracker.beginUserTurn()` to drop prior-turn live workers **and** pending unbound spawn/finish receipts while keeping scheduled tasks; `subagentSpawnedEvents` / `subagentFinishedEvents` retain exact-once, generation-bound authoritative receipts; `unboundSubagentSpawnedEvents` reads unmatched pending spawn receipts |
| Chat UI | **Run inspector** (`ActivitySidebar`, header toggle) shows spawn-created workers plus unbound spawn receipts with terminal metadata, wait receipts, and explicit unknown/orphaned/cancelled status; the Tasks pill uses the same live mirror plus unbound spawn menu rows |
...
| `ChatView.swift` | Centered work transcript, one-to-eight-line matte composer, visible command/submission controls, Ask/Build/Review welcome with on-canvas outcome copy, gated task-contract strip, Resume/Start/Browse on restored transcripts; header **Run inspector** toggle |
...
Review chip and the inline changed-files card; the live Run row's Run inspector control
is wired, and it has no Review no-op. The Run inspector remains the opt-in ledger. Artifacts retain exact local path, parent tool ID, optional worker ID, and
...
| **Scheduled tasks / runtime lease** | `ScheduledTaskStore.swift`, `SessionRuntimeRetentionPolicy` in `SessionProcessIdentity.swift`, `ChatStore.scheduledTasks` / `scheduledTaskInventoryReceipt` / `runtimeLease`, `ContentView.enforceConnectionCap`, `SessionDashboardPanel`, Tasks menu, and `AcpEvent.schedulerActivity` |
| **Background tasks / subagent coordination** | `BackgroundTaskStore.swift` (single permutation-proof reducer + per-turn metrics), `ChatStore.backgroundActivities`, `ActivitySidebar`, `AcpEvent.backgroundActivity` / typed subagent lifecycle |
...
| `ACPClientContractTests.swift` | Terminal lifecycle, bounded UTF-8 output, command compatibility, tool failure parsing, generation-bound model reducer/ACP fixtures, nested ACP session-mode parsing, split MCP `serverName`+`toolName` qualified identity before `rawOutput`, idle inherited Default vs Unknown, welcome-on-draft, model fallback, composer targets, static progress, off-main settings work, restored-view bottom-follow, updater freshness, and workbench-not-chatbot source contracts |
