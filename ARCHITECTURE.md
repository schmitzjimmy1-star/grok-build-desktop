...
`ChatStore.continuityRequiresRecovery` is the hard-block predicate — `true` only for `.diverged`, `.compositeSuspected`, `.backendMissing`, and `.verificationIncomplete`. `ChatStore.continuityIsResuming` is `true` only for `.verifying`. The Send button's `.disabled(...)`, `.help`, `.accessibilityLabel`/`Hint`, the VoiceOver "needs attention" announcement, and the Activity snapshot's `continuity.requiresRecoveryAction` all bind to these predicates instead of the raw `continuityBlocksSend`. A restored tab that is resuming keeps Send enabled with hopeful copy ("Send to resume this saved session"); a genuinely diverged/missing backend still disables Send and points to the drawer's recovery review. `continuityBlocksSend` remains only for the internal `deliverPrompt`/gate plumbing. Covered by `LifecycleAndSubprocessTests.testVerifyingKeepsSendUsableWhileRecoveryStatesBlockIt` and `…testSendButtonBindsToRecoveryPredicateNotRawGate`.

The state is also made visible inline. `ContinuityStatusBanner` (`ComposerViews.swift`) renders directly above the composer, alongside `GoalBanner`/`BtwAsideBanner`: `.resuming` is a calm one-line hint while `continuityIsResuming`, and `.needsRecovery` shows the `continuityHeadline`/`continuityMessage` with a one-click **Review** button when `continuityRequiresRecovery`. Review reuses the existing explicit flow (`showRecoveryReview` + `ChatStore.reviewRecoveryCandidates` → `SessionRecoveryReviewSheet`); the drawer path is unchanged. Tab **selection remains presentation-only** — it never starts a backend or verifies (`ContentView.selectSession`; pinned by `ACPClientContractTests`) — so browsing a saved session does not spawn a process or consume an LRU slot; resume happens when the user sends. Inline banner wiring is covered by `LifecycleAndSubprocessTests.testComposerSurfacesContinuityStatusBannerInline`.
