...
| Store | `ChatStore.backgroundActivities` is the correlated presentation projection; `clearTurnState` calls `BackgroundTaskTracker.beginUserTurn()` to drop prior-turn live workers while keeping scheduled tasks; `subagentSpawnedEvents` / `subagentFinishedEvents` retain exact-once, generation-bound authoritative receipts |
| Chat UI | **Run inspector** (`ActivitySidebar`, header toggle) shows every spawn-created worker with terminal receipt metadata, wait receipts, and explicit unknown/orphaned status; the Tasks pill uses the same live mirror |
