...
pass instead of cancel/restarting the six-pass window. While the transcript is
auto-following a live or settling turn, `ChatTranscriptSelectionPolicy` also
suspends AppKit text selection on settled rich Markdown without dropping to plain
`Text`; a reader who scrolled away keeps copy. Attached `streamRevision` updates
skip the per-chunk `scrollTo` and keep that coalesced settlement window.
Transcript rows use composite
...
pager; they skip in CI and never replace `~/.grok/bin/grok`. Slice 4B.6
byte-copies that same signed pager into an owner-private
...
Two parallel updaters — **GrokBuild app** (GitHub) and **grok CLI** (`grok update`). UI surfaces: the standard GrokBuild application menu, the quiet workspace-rail indicator, Settings, and `UpdatePanel`.
...
- **Rail indicator:** `grok-upgrade-indicator` in `SidebarView` opens
  `UpdatePanel`; its context menu retains explicit dismissal until next launch
- **Panel:** dual sections; mutual busy lock during install

### DEBUG simulate updates

Menu **Simulate Updates** (`#if DEBUG` only — use `make run-debug`, not `make run`): fake `99.0.0` pending updates and show the rail update indicator only. Same discovery flow as real updates — open the update panel from the indicator. Simulated app install relaunches GrokBuild without replacing the binary; simulated CLI updates never run `grok update`. **Clear Simulation** runs real checks.
...
```
┌─────────────────────────────────────────────────┐
│ SidebarView 256 │ ChatView │ Preview/Run (opt)  │
│ update indicator│ centered │ peer dock/overlay  │
├─────────────────────────────────────────────────┤
│ SettingsView (replaces chat when open)          │
└─────────────────────────────────────────────────┘
```

F5E makes the optional parts of that diagram truthful. The trailing Run control
mounts only when a live projection, settled snapshot, or already-open evidence
drawer exists; an idle New chat has no dead inspector menu. Workspace, branch,
and saved-task Resume/New/Browse actions share one context bar above the composer
without changing their `ChatStore` owners. The welcome asks **What should we
build?** and renders outcome-visible Ask/Build/Review starters. The 256-point rail
uses flat hover-only Plugins/Security utilities, 14-point conversation titles,
38-point session targets, and explicit removal of stale selected accessibility
traits. `SettingsLoadStateView` renders successful-empty inventory copy without a
Retry action; stale/error states retain Retry and the state model is unchanged.

### Key views

| File | Role |
|------|------|
| `AppTheme.swift` | Adaptive near-black Dark and white/warm-mist Light canvases, typography, layout widths, radii, semantic accent/foreground/warning/link tokens, app-owned primary/chrome button styles, accessible native-material surface, and `TitlebarGlyph` / `titlebarControl` so transparent-titlebar icons stay readable on Dark |
| `SidebarView.swift` | 256-point navigation rail: shared 46-point brand row, quiet update indicator, New chat, flat hover-only Plugins/Security utilities, selected-project sessions with 14-point titles and 38-point targets, pins, roomy Recents discovery, on-demand filter, model/running/last-used metadata, hover/context rename and close actions, and the sole Settings entry in the account row (`grok-sidebar-account-settings`). There is no duplicate top-level Sessions route. Session rows expose `grok-sidebar-session-row-<tabUUID>` and accessibility value equal to that UUID so 4B.4 continuation can select one retained tab. Utility controls explicitly remove stale selected traits. Filter and the **Session dashboard** bell live in `ChatTopBar`, to the right of the session title. Command-comma still works. The former permanent Activity lane, Agents hub, and Connections sections were removed; their capabilities live in the evidence-gated Run inspector / session dashboard, Settings → Agents, and the composer MCP menu |
| `ChatView.swift` / `ConversationWorkbench.swift` | Centered work transcript and presentation-only assistant-turn spine; Ask/Build/Review welcome with visible outcomes on genuine New chat; **Loading saved conversation…** plus Resume/Start/Browse inside the unified project context bar on restored tabs (including empty-hydrate); evidence-gated Run inspector, checkpoint-derived outcome badge, turn-bound changed-files handoff, composite transcript-block identity, coalesced settled scrolling, attached-stream skip of per-chunk follow scrolls, and the non-selectable restore-transition snapshot. `ChatView` remains the state/action owner and hosts thin `topBar` / `composer` / `headerReviewToggle` wrappers |
| `ChatTopBar.swift` | Workbench header and project menu just under the traffic lights. Session title stays leading; Filter, Session dashboard, and More actions sit to the right with a gap. Tasks and Review remain ChatView-owned slots; Run mounts only for live, settled, or already-open evidence. No header hairline. Icons use `TitlebarGlyph` (baked near-white on Dark). Settings is not in this bar. |
...
| **Empty/welcome state, work intents** | `WelcomeStateView.swift` (quiet workspace welcome + outcome-visible intent starters), `ChatView.swift` (`restoredEmptyState`, `noProjectState`, unified composer context, mount/send gate), `ChatStore.showsEmptyTranscriptWelcome` / `isResumedSessionTab`, `WorkbenchIntent` in `ComposerModels.swift` |
...
| `AcceptanceHarnessTests.swift` | Agentic `scripts/acceptance/` harness: dry-run default, `--billable` fail-closed without a run ID, guessed-cleanup refusal, fixture-mode reject/accept cases at zero provider cost, Resume-then-Send labels, installed-exec refuse of `.build` / `dist`, Slice 6 250k Stop packet, 4B.4 fresh-process continuation schema/receipt/governed-load driver contracts, schema-3 dry-run, schema-3 `--billable` still ceiling-locked, 4B.5 loopback/driver source pins, and 4B.6 install/rollback/paid-lock Python contracts |
| `Slice4B5LifecycleTests.swift` | Owner-local (`GROKBUILD_SLICE4B3_RUNTIME_SELECTION`) nonbillable staged-pager loopback: 4B.6 class setup byte-copies the signed pager into an owner-private dest, then reruns normal sentinel, redirect/retry zero, stop/cancel, kill-after-reserve, kill-after-response-before-settlement (`hold_after_body`), kill-during-restart, missing usage, stream failure, call ceiling, ordered reads, worker/wait, recovery, continuation `session/load` against that installed copy; plus CI-safe resolver/paid-lock pins; skips the live pager matrix in CI; never replaces `~/.grok/bin/grok` |
...
| Mount per-block AppKit text selection on settled Markdown during auto-follow streaming or settlement | `ChatTranscriptSelectionPolicy` suspends selection while following live/settling turns; keep rich Markdown and restore copy after settle |
...
**Armed v3 authority checkpoint (Slice 4B.3, T5 proven locally, nonbillable).** The CLI
...
`authScheme`. The full `ExecutionExpectation` match remains unreferenced by
launch and Keychain. Armed `initialize` uses `parseInitializeProvenance` plus
the Swift-observable `ArmedV3DispatchExpectation` subset, not an invented
`ResolvedRouteBound`. `isEnforcing` stays the only historical v2 packet path.
...
live items. Schema-3 `AcceptanceBudgetGuard` packets still parse
`credentialAuthorizationV3` selectors (managed provider, scheme, bound digest).
Dispatch does not treat that packet object as authority: `ArmedV3DispatchExpectation`
cross-binds those selectors to the live selected custom model and linked
provider (`authBoundary == .officialHelper`) and only then fills
`HardBudgetLaunchContract`, including the live `dispatchExpectation`. Spawn
...
Attached bottom-follow still throttles live chunk scrolling and respects deliberate reader detachment. An already-attached viewport skips the per-chunk `scrollTo`; `ChatAutoScrollPolicy` keeps one coalesced six-pass settlement window through approximately 3.1 seconds after the latest content event so later-turn Markdown/WebKit intrinsic-height changes cannot strand the final lines behind the composer. Newer milestones request a trailing pass instead of cancel/restarting that window, and the task exits immediately for a reader who deliberately scrolled away; no periodic invalidation or unbounded follow loop was added.
