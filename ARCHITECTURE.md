...
The composer label and the keyboard-reachable session receipt distinguish Saved, Default, Pending, Requested, Live, Last live, Rejected, and Unknown. An idle inherited New chat with a catalog model and no process yet is Default, not Unknown. Unknown stays for a truly empty picker or a live process with no model receipt. Last live applies only when the picker is still on the confirmed receipt; an inherited tab that followed a newer CLI or project default is Default, not Last live on the wrong model. Launch arguments and picker intent prove only what GrokBuild requested; when ACP omits effective state the details explicitly say the backend model is not independently exposed. Restore no longer emits a hidden second `session/set_model` to force the picker to match saved state.
...
| `SidebarView.swift` | Navigation-only (Codex parity Slice 1): New chat/Sessions/Plugins/Security rail, collapsible project/session hierarchy, pins, on-demand filter, model/running/last-used metadata, hover/context rename and close actions, header Activity bell, and an account footer. Settings opens from the session header and Command-comma. The former permanent Activity lane, Agents hub, and Connections sections were removed; their capabilities live in the Activity drawer/dashboard, Settings → Agents, and the composer MCP menu |
| `ChatView.swift` | Centered work transcript, one-to-eight-line matte composer, visible command/submission controls, Ask/Build/Review welcome with on-canvas outcome copy, gated task-contract strip, Resume/Start/Browse on restored transcripts |
