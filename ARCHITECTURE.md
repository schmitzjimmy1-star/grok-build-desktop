...
**Workbench polish — 2026-08-03.** Four coordinated arrangement contracts: (1) **fresh sessions warm-start** — `createLiveSession` (non-resume only) launches the grok process in the background at creation, because creating a session is explicit intent to work; the first send no longer pays the multi-second launch (session/new, MCP wiring, model readback). Resume paths are untouched, the tab stays `localOnly` so no continuity gate is bypassed, and the LRU cap still applies. (2) **Model menus are provider-grouped** (`ChatStore.groupedAvailableModels`: "Grok" natives vs "Your models" from config.toml; empty groups render nothing) so custom/OpenRouter routes read as first-class main-agent choices, and every model menu offers **Use Current Model for New Sessions in This Project** (`setCurrentModelAsProjectDefault` → per-workspace `SessionLayoutStore.agentSettings.model`, the exact source `inheritProjectDefault` resolves; posts `.workspaceAgentSettingsChanged`; no restart, no config.toml write). (3) The composer Details row keeps its **leading edge pure telemetry** (context gauge + session usage HUD) and moves the **agent-mode selector into the trailing action cluster** beside review/Activity. (4) The Activity drawer **hides empty sections** (settled artifacts/review files/workers; live plan/artifacts/workers/tools) — the summary grid already reports zeros, so "none observed" rows below it were pure redundancy. Pinned by `UsageAndRoutingTests.testWorkbenchPolishWiring` and the updated `WorkbenchIntentTests`.

**Consumer surfaces — Slices 7+8 (2026-08-03).** The Activity drawer speaks plain language: "What the agent did" / "Happening now — not final" / "Finished" badge, "Technical details" (was Execution receipts), "Session details" (was Live binding), "No final report"/"No final report (orphaned)"/"Not finished yet" statuses, and human summary lines ("Everything finished and checked out.", "Finished with 1 unresolved error."). The unknown/orphaned distinction and every truth boundary survive — only the vocabulary changed. When no run evidence exists the drawer shows an **idle Workspace panel** instead of a dead placard: the project's changed files (click → diff review, magnifier → reveal in Finder) or a ready-to-work explainer; run receipts take the space back the moment a turn starts (`idleChangedFiles`/`onOpenReview`/`onRevealFile` props fed from ChatView's existing review state). The composer's wide row fills its dead middle with `composerCenterHint` — the session usage meter once turns settle, keyboard affordances before that (moved out of the Details row). `showsEmptyTranscriptWelcome` replaces the old `isResumedSessionTab` welcome gate so a warm-started or restored-but-empty tab shows the Ask/Build/Review welcome instead of a void (only a hard continuity block hides it). `ModelAPIBackend.displayName` uses consumer labels ("Standard chat (OpenAI-compatible)" / "OpenAI Responses" / "Anthropic Messages").

**Sidebar Agents hub (agentic roadmap Slice 2).** `AgentHubProjection.entries` (`Models/AgentHub.swift`) projects grok's built-in Default, custom subagent roles (sorted, with their model hint or "Inherits session model"), and discovered agents (sorted; a same-named role wins the collision) into an **Agents** sidebar section, flagging the Settings default-for-new-sessions row. Activating a row starts a new session in the selected project with the agent bound as an explicit `TabAgentIntent` **before** any process exists (`ContentView.createLiveSession(for:agent:)` → `bindTabSession(agentIntent:)`), so the lazy first-send launch carries `--agent` naturally — no restart of a fresh session. Roles reload off-main at bootstrap and on `.subagentRolesChanged`; discovered agents ride the active store's per-workspace `loadDiscoveredAgentsIfNeeded` guard. Context menu offers Manage Agents… (Settings → Agents). Covered by the `testAgentHub*` cases in `AgentsAndCapabilitiesTests`.
...
| **Session usage / cost HUD** | `SessionUsageLedger` (`Models/SessionUsage.swift`), `ModelPricingStore`, `ChatStore.sessionUsage` / `sessionUsageSummary`, Details bar in `ChatView` |
...
`ChatStore.refreshPromptMCPOptions` reads only secret-free enabled connection metadata from `grok mcp list --json`, then merges observed runtime MCP status by exact name. Selection is in-memory, per tab, and consumed only after a prompt is accepted for delivery. `PromptMCPAttachmentPromptBuilder` sanitizes, deduplicates, and sorts selected names into a bounded instruction that explicitly forbids claiming use when no attached MCP tool actually ran. The composer exposes the resulting selection as removable chips and orders the primary controls MCP, hammer, Details, then model. Connection commands, URLs, headers, environment, and credentials never enter this surface or the transcript.

MCP use is evidence-bound. `GrokProcess.parseToolCall` preserves the provider's authoritative `serverName` / `server_name`, qualified tool name, and redacted title alongside the already-redacted tool input. `ChatStore.LiveToolCall`, `RunEvidenceLiveProjection.Tool`, and the optional `AssistantTurnTrace.Tool` propagate the explicit server name. When an ACP route omits that field, ChatStore accepts the bounded safe namespace from the provider-qualified `<server>__<tool>` name/title receipt, preferring the longest exact configured/runtime MCP match when available. Prompt selection and inventory presence alone never synthesize use. Consequently **Using <server>** always requires a current tool receipt, while an unqualified tool gets its ordinary title with no inferred MCP claim.
