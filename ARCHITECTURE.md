...
| `ActivitySidebar.swift` | Codex parity Slice 5: compact contextual inspector (content-height top-trailing overlay) driven by the pure `ContextInspectorProjection` (`Models/ContextInspectorProjection.swift`) — recovery card first, then Subagents counts, gated Computer Use with a Settings link, Sources separating requested from receipt-evidenced MCPs, and a non-collapsible unresolved-errors line; the full generation-bound `Live`/`Settled` evidence stack survives unchanged inside one Run details disclosure; Escape/X/header toggle all close it; no lifecycle or worker state lives in SwiftUI |
